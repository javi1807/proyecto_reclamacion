...
class Sugerencias:
    ...
    def __init__(self, window, ver=False):
        ...
        frame = tk.LabelFrame(self.wind, text='SUGERENCIAS\nIngrese su sugerencia', labelanchor='n', bg='#008080', fg='#ffffff', font=('Arial', 12, 'bold'))
        ...
        if not self.ver:
            ...
            tk.Label(frame, text='A continuación ingrese su sugerencia detalladamente:', bg='#008080', fg='#ffffff').pack(anchor=tk.W, pady=5)
            self.sugerencia = tk.Text(frame, height=6, font=('Arial', 12))
            self.sugerencia.pack(fill=tk.BOTH, pady=5)
            ...
            ttk.Button(button_frame, text='Ver sugerencias', command=self.ver_sugerencias, style='TButton').pack(side=tk.RIGHT, expand=True, fill=tk.X, padx=5)

            ttk.Button(frame, text='Guardar sugerencia', command=self.add_sugerencia, style='TButton').pack(fill=tk.X, pady=5)
    ...
    def validation(self):
        return len(self.author.get()) != 0 and len(self.sugerencia.get("1.0", tk.END).strip()) != 0

    def add_sugerencia(self):
        if self.validation():
            query = 'INSERT INTO Sugerencias VALUES(NULL, ?, ?, ?)'
            parameters = (self.author.get(), self.sugerencia.get("1.0", tk.END).strip(), self.date.get())
            self.run_query(query, parameters)
            self.message['text'] = f'Sugerencia {self.author.get()} añadido exitosamente'
            self.author.delete(0, tk.END)
            self.sugerencia.delete("1.0", tk.END)
            self.date.delete(0, tk.END)
            self.date.insert(0, datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        else:
            self.message['text'] = 'Autor y sugerencia son requeridos'

    def ver_sugerencias(self):
        self.new_window = tk.Toplevel(self.wind)
        self.new_window.geometry("800x600")
        self.new_window.title('Ver sugerencias')

        tree_frame = ttk.Frame(self.new_window)
        tree_frame.pack(fill=tk.BOTH, expand=True)

        self.tree = ttk.Treeview(tree_frame, height=15, columns=('sugerencia', 'fecha'))
